...
export class BetsValidationService {
    ...
    public async validateMatchBets() {
        ...
        const updatedGames: Game[] = await this.gamesService.getUpdatedGamesByStages(gameStagesToValidate);

        if (!updatedGames.length) {
            return;
        }

        const updatedGameBets: GameBet[] = await this.gameBetService.updateBetsByGames(updatedGames);
        ...
        const updatedUsers = await this.recalculateService.recalculateUsersScores(updatedBets.map(({createdBy}) => createdBy._id));
    }

    private async getGameStagesToValidate(): Promise<string[]> {
        const stages = await this.gameRepository.getMany({
