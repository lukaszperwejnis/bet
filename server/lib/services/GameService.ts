...
export class GameService {
    ...
    public async addScheduledMatchesToDatabase() {
    ...
    private mapToGames(matches: ExternalGame[]): Promise<Game[]> {
        return Promise.all(
            matches.map(async ({homeTeam, awayTeam, score, ...values}) => {
                const homeTeamDoc = await this.teamRepository.getOne({externalId: homeTeam.externalId});
                const awayTeamDoc = await this.teamRepository.getOne({externalId: awayTeam.externalId});
                return {
                    ...values,
    ...
    private async createGames(matches: Game[]): Promise<Game[]> {
        ...
        for (let betToAdd of matches) {
            const game = await this.gameRepository.getOne({externalId: betToAdd.externalId});
            if (!game) {
    ...
    public async getUpdatedGamesByStages(stages: string[]): Promise<Game[]> {
        const finishedGames = await this.externalGamesService.getGamesByStages(stages.join(','))
            .then(data => data.filter(game => game.status === GameStatuses.FINISHED));
    ...
    public async getAvailableByUserId(userId: string): Promise<Game[]> {
