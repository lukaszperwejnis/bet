...
export const ResetPassword = (): ReactElement => {
    ...
    const schema = Yup.object({
        email: Yup.string().email(translate('validation.email.syntax')).required(translate('validation.email.required')),
    });
    ...
    const ResetPasswordForm = (): ReactElement => (
        <Formik validationSchema={schema} initialValues={{email: ''}} onSubmit={onSubmit}>
            <Form>
    ...
    const SuccessInfo = (): ReactElement => <FormattedMessage id="resetPassword.success" />;

    return (
        <Page centered>
            <PageTile header={<FormattedMessage id="resetPassword.header" />}>{isSuccess ? <SuccessInfo /> : <ResetPasswordForm />}</PageTile>
        </Page>
    );
};
