...
function* login(action) {
    try {
        const { data } = (yield effects_1.call(_services_1.authService.login, action.payload));
        ...
        yield effects_1.call(_utils_1.redirect, _config_1.AppUrls.DASHBOARD.pattern);
    }
    catch (error) {
...
function* signup(action) {
    try {
        ...
        yield effects_1.call(message_actions_1.messageActions.success, _utils_1.translate('signup.success'));
...
function* authSaga() {
    yield effects_1.all([effects_1.fork(watchLogin), effects_1.fork(watchSignup)]);
}
