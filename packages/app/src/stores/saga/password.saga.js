...
function* startReset(action) {
    try {
        yield effects_1.call(_services_1.passwordService.startReset, action.payload);
...
function* reset(action) {
    try {
        ...
        yield effects_1.call(message_actions_1.messageActions.success, _utils_1.translate('resetPassword.success'));
...
function* passwordSaga() {
    yield effects_1.all([effects_1.fork(watchStartReset), effects_1.fork(watchReset)]);
}
